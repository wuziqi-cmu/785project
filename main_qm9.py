...
parser.add_argument('--lr', type=float, default=1e-3, metavar='N',
                    help='learning rate')
parser.add_argument('--nf', type=int, default=10, metavar='N',
                    help='learning rate')
...
parser.add_argument('--property', type=str, default='energies', metavar='N',
                    help='label to predict: alpha | gap | homo | lumo | mu | Cv | G | H | r2 | U | U0 | zpve')
parser.add_argument('--num_workers', type=int, default=0, metavar='N',
                    help='number of workers for the dataloader')
parser.add_argument('--charge_power', type=int, default=1, metavar='N',
...
model = EGNN(in_node_nf=4, in_edge_nf=0, hidden_nf=args.nf, device=device, n_layers=args.n_layers, coords_weight=1.0,
...
lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", factor=0.95, min_lr=1e-9, patience=15,
                                                          verbose=True)
myLoss = nn.L1Loss()
# wtfmyLoss = nn.L1Loss()
...
def train(epoch, loader, partition='train'):
    ...
    for i, data in enumerate(loader):
        ...
        if partition == 'train':
            wtfloss = wtfmyLoss((pred - meann)/mad, (label-meann)/mad)
            loss = myLoss(pred * -1, label * -1)
            # loss = myLoss(pred, (label-meann)/mad)
            loss.backward()
            optimizer.step()
        else:
            wtfloss = wtfmyLoss((pred - meann)/mad, (label-meann)/mad)
            loss = myLoss(pred * -1, label * -1)
            # loss = myLoss((pred * mad)+meann, label)
        ...
        res['wtfloss_arr'].append(wtfloss)

        if partition == 'train':
            if i % args.log_interval == 0:
                loss_to_print = sum(res['loss_arr'][-10:]) / len(res['loss_arr'][-10:])
                lr_scheduler.step(loss_to_print)
        ...
        if i % args.log_interval == 0:
            loss_to_print = sum(res['loss_arr'][-10:]) / len(res['loss_arr'][-10:])
            wtfloss_to_print = sum(res['wtfloss_arr'][-10:]) / len(res['wtfloss_arr'][-10:])
            print(prefix + "Epoch %d \t Iteration %d \t loss %.4f \t loss %.4f" % (epoch, i, loss_to_print, wtfloss_to_print))
        if i == myL - 1:
            print(label[:10])
            print(pred[:10])
            # print(pred[:10] * mad + meann)
            print()
