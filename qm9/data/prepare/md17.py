...
md17_subsets = {'benzene': 'benzene2017_dft',
               'uracil': 'uracil_dft',
...
def download_dataset_md17(datadir, dataname, subset, splits=None, cleanup=True):
    ...
    if splits is None:
        ...
        canis = random.sample(range(0, num_tot_mols-1), np.sum(splits))
        tempSplits = {'train':torch.tensor(canis[:splits[0]]), 'valid':torch.tensor(canis[splits[0]:splits[0]+splits[1]]), 'test':torch.tensor(canis[splits[0]+splits[1]:])}
        splits = tempSplits
